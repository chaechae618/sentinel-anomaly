...
def isolation_forest(series):
    ...
    n = len(series)
    window = min(50, n // 4)
    X = np.array([series[max(0, i-window):i+1].tolist() + [i/n] for i in range(n)])
    # Use a simple sliding window feature
    feats = []
    for i in range(n):
...
def matrix_profile_discord(series, m=20):
    ...
    if m >= n // 2:
        m = max(4, n // 4)
    # Approximate matrix profile using STOMP-lite (simplified)
    scores = np.zeros(n)
    subs = []
    for i in range(n - m + 1):
        s = series[i:i+m]
        mu, std = s.mean(), s.std() + 1e-9
        subs.append((s - mu) / std)
    subs = np.array(subs)
    # For each subsequence find nearest neighbor distance
    for i in range(len(subs)):
        dists = []
        for j in range(len(subs)):
            if abs(i - j) > m // 2:
                d = np.linalg.norm(subs[i] - subs[j])
                dists.append(d)
        if dists:
            scores[i] = min(dists)
    # Extend last m-1 points
    if len(scores) > m:
        scores[-(m-1):] = scores[-(m):-1] if m > 1 else 0
    return scores / (scores.max() + 1e-9)
...
def inject_anomalies(series, seed=42):
    ...
    vend = vstart + rng.integers(10, 20)
    vend = min(vend, n)
...
def point_adjusted_f1(preds, labels):
    # if any point in anomaly window detected, credit whole window
...
def analyze():
    try:
        ...
        for var in selected_vars:
            ...
            # per-variable scores from each detector
            scores_per_det = {}
            thresholds = {}
            
            # synthetic injection for evaluation
            ...
            # AR forecast line for visualization
            p_ar = min(10, n // 4)
            try:
                X_ar, y_ar = [], []
                ...
                y_ar = np.array(y_ar)
                lam = 1.0
                coef = np.linalg.solve(X_ar.T @ X_ar + lam * np.eye(p_ar), X_ar.T @ y_ar)
        ...
        # global consensus distribution
        vote_dist = {}
        for k in range(len(active_detectors) + 1):
            vote_dist[str(k)] = int((all_consensus_votes == k).sum())
